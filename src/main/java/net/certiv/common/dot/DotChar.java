...
public class DotChar {
	 ...
	 * . period -> \0225 bullet
	 * - dash   -> \0226 en dash
	 * # hash   -> \0244 currency sign
	 * & amp    -> \0247 section sign
	 *
	 * ( -> \0223 left double quotation
	 * ) -> \0224 right double quotation
	 * [ -> \0246 broken vertical bar
	 * ] -> \0246 broken vertical bar
	 * < -> \0213 single left-pointing angle quotation
	 * > -> \0233 single right-pointing angle quotation
	 * { -> \0253 double left-pointing angle quotation
	 * } -> \0273 double right-pointing angle quotation
	 *
	 * ? -> \0227 em dash
	 * ? -> \0255 soft hyphen
	 * </pre>
	...
	private static int fx(int cp) {
		switch (cp) {
			case ' ':
				return 0230; // small tilde
			case ',':
				return 0202; // low single quote mark
			case '.':
				return 0225; // bullet
			case '-':
				return 0226; // en dash
			case '#':
				return 0244; // currency sign
			case '&':
				return 0247; // section sign
			case '(':
				return 0223; // left double quotation
			case ')':
				return 0224; // right double quotation
			case '[':
			case ']':
				return 0246; // broken vertical bar
			case '<':
				return 0213; // single left-pointing angle quotation
			case '>':
				return 0233; // single right-pointing angle quotation
			case '{':
				return 0253; // double left-pointing angle quotation
			case '}':
				return 0273; // double right-pointing angle quotation
