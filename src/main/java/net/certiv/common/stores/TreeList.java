...
/**
 * Sorted multimap implemented as a TreeMap with LinkedList implemented instance
 * values. Keys are implicitly unique.
 *
...
public class TreeList<K, V> {
	...
	/**
	 * Constructs a new, empty treelist, using the natural ordering of its keys. All
	 * keys inserted into the treelist must implement the {@link Comparable}
	 ...
	 * keys {@code k1} and {@code k2} in the treelist. If the user attempts to put a
	 * key into the treelist that violates this constraint (for example, the user
	 * attempts to put a string key into a treelist whose keys are integers), the
	...
	/**
	 * Constructs a new, empty treelist, ordered according to the given comparator.
	 * All keys inserted into the treelist must be <em>mutually comparable</em> by
	 * the given comparator: {@code comparator.compare(k1, k2)} must not throw a
	 * {@code ClassCastException} for any keys {@code k1} and {@code k2} in the
	 * treelist. If the user attempts to put a key into the treelist that violates
	 * this constraint, the {@code put(Object key, Object value)} call will throw a
	 * {@code ClassCastException}.
	 *
	 ...
	 * @throws ClassCastException if the keys in m are not {@link Comparable}, or
	...
	/**
	 * Returns the value list identified by the given key or {@code null} if the key
	 * does not exist.
	 */
	 ...
	 * {@link enforceUniqueValues} is set to {@code true}, the value is added only
	 * if value is not already present (based on object equality).
	 */
	...
	/**
	 * Puts the given value at the beginning of the value list identified by the
	 * given key.
	 */
	...
	/**
	 * Puts the given values at the beginning of the value list identified by the
	 * given key. The order of the individual added values is preserved.
	 */
	 ...
	 * @throws NullPointerException if the specified map is null or the specified
	...
	/**
	 * Returns the first value in the list identified by the given key or
	...
	/**
	 * Returns the last value in the list identified by the given key or
	 * {@code null} if the list is empty.
	 */
