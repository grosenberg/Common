...
public class Log2 {
	...
	public static void printf(Level level, String fmt, Object... args) {
		...
		if (loggable(origin, level)) {
			...
			logger.logMessage(FQCN, level, null, new StringFormattedMessage(fmt, args), null);
		}
	}
	...
	private static Class<?> caller() {
		StackFrame frame = StackWalker.getInstance(OPTIONS).walk(Log2::caller);
		return frame != null ? frame.getDeclaringClass() : Log2.class;
	}
	...
	private static final String FQCN = Log2.class.getName();
	private static final int LogId = Log2.class.hashCode();
	...
	private static Class<?> refCls_ = Log2.class;
	...
	public static void defLevel(Level level) {
		setLevel(Log2.class, level != null ? level : defaultLevel());
	}
	...
	private static void setLevel(Class<?> cls, Level level) {
		...
		if (cls == Log2.class) {
			trace("Default logging level set to [%s]", level);
		} else {
			trace("Logging level set for %s[%s]", cls.getName(), level);
		}
	}
	...
	private static void chkInit() {
		if (!initd_) {
			...
			if (ctx_ != null) {
				...
				error("Configuration failed @%s[%s:%s]", logname_, location_, layout_);
			}
		}
	}
